...
- Add migration `2026091501_jobs_list_index` — index on `jobs (project_id, queue_name, created_at DESC)` so control/cloud job listing no longer full-scans large queues (fixes multi-second `ListJobs` under load)
